...
#define CURRENT_INIT 2.95  // Adjust this to the current reading at 0mm depth (unit: mA)
#define DENSITY_WATER 1    // Pure water density normalized to 1
#define PRINT_INTERVAL 1000
#define SEND_INTERVAL 13300
...
void loop() {
  if (millis() - timepoint_measure > PRINT_INTERVAL) {
    ...
    depth = (dataCurrent - CURRENT_INIT) * (RANGE / DENSITY_WATER / 16.0)+18;  // Calculate depth from current readings
    ...
    int litrosTotal = (averageDepth * 33000) / 140;
    ...
    if (millis() - timepoint_measure2 > SEND_INTERVAL) {
      ...
      sendDataToThingSpeak(averageDepth, litrosTotal);
    }
  }
}

void sendDataToThingSpeak(float depthValue, int litrosTotal) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;

    // Modify the URL to include both field1 and field2
    String url = String(serverName) + "?api_key=" + apiKey + "&field1=" + String(depthValue) + "&field2=" + String(litrosTotal);
    http.begin(url);
